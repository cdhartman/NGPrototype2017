	...
	$(node).each(function (i,  row) { 
		...
		if ($(this).attr('name') !== undefined  && i >= start && columnNumber < categoriesPerScreen) {
			...
			if (view === 'Adult' || view === 'Pediatric') {
				constructURL = 'mciGenre.html?genre=' + encodeURI(genre) + '&subGenre=' + encodeURI(subGenre) + '&genreID=' + genreId + '&subGenreID=' + $(this).attr('id') + '&view=' + view;
				lineColumnMainWideFolderClass = 'lineColumnMainWideMciFolder';
			} else if ($(this).attr('exitAndTune') === 'false') {
				// constructURL = 'mciSubCat.html?genre=' + encodeURI(subGenre) + '&genreID=' + $(this).attr('id') + '&view=' + view;
				constructURL = 'mciGenre.html?genre=' + encodeURI(genre) + '&subGenre=' + encodeURI(subGenre) + '&subGenreID=' + $(this).attr('id') + '&view=' + view;
				lineColumnMainWideFolderClass = 'lineColumnMainWideMciFolder';
			} else if ($(this).attr('hasCategories') === 'false') {
				constructURL = 'mciGenre.html?genre=' + encodeURI(genre) + '&subGenre=' + encodeURI(subGenre) + '&genreID=' + genreId + '&subGenreID=' + $(this).attr('id') + '&view=' + view;
				lineColumnMainWideFolderClass = 'lineColumnMainWideMci';
			} else { 
				constructURL = 'mciTitle.html?genre=' + encodeURI(genre) + '&genreID=' + encodeURI(genreId) + '&subGenre=' + encodeURI(subGenre) + '&subGenreID=' + encodeURI(subGenreId) + '&view=' + view + '&offeringId=' + $(this).text();
				lineColumnMainWideFolderClass = 'lineColumnMainWideMci';
			}
			...
			divContent += ' onFocus=\"onHoverDiv(\'onFocus\', \'divLink' + count + '\' ,\'left\',\'false\',' + rowNumber + ',' + columnNumber + '); offSetFunction(\'divLink' + count + '\');\"';
			divContent += ' onBlur=\"onHoverDiv(\'onBlur\', \'divLink' + count + '\' ,\'right\',\'false\',' + rowNumber + ',' + columnNumber + ');\"';
			divContent += '>';
